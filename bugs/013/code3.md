...
- **Implementations**:
  ...
  - **[TOCTOU Protection]**: 
    - Introduced a package-level mutex `setMemoResourcesMu` to serialize all invocations of `SetMemoResources` (the only endpoint capable of moving or unbinding existing resources' `MemoID`).
    - Added a pre-update query checking each resource's current DB state inside the update loop. If a previously-exempted resource was concurrently reassigned/unbound, `checkResourceAccess(ActionWrite)` is triggered to prevent unauthorized hijacking.
...
--- PASS: TestResourceReadAccessUnattached (0.04s)
...
PASS
ok      github.com/usememos/memos/server/router/api/v1  0.554s
```
