# Plan v2: bchat E2E Critical Path Testing
...
**Approach:** curl-based shell scripts, critical path only (~27 scenarios across 11 groups)
...
|---|------|-------------|-----------|-----------------|
| 1.1 | Sign up as first user | `POST /api/v1/auth/signup` | User creation, JWT cookie, HOST role | `role == "HOST"`, `username == "admin"` |
| 1.2 | Sign in + auth status | `POST /api/v1/auth/signin` + `POST /api/v1/auth/status` | Cookie auth, JWT validation | `username` present in status response |
| 1.3 | Sign out | `POST /api/v1/auth/signout` | Cookie clearing | Response is `{}` |

**Note:** Test 1.1 handles re-runs — if signup returns 409/400 (user exists), falls back to sign-in.

### Group 2: Tenant Lifecycle (3 tests)

| # | Test | API Endpoint | Validates | JSON Assertions |
|---|------|-------------|-----------|-----------------|
| 2.1 | Onboard tenant with KB/Policy | `POST /api/v1/agent/onboard` | Tenant creation, file parsing | `success == true`, `tenant.slug` present |
| 2.2 | Get tenant config | `GET /api/v1/agent/:slug/config` | Config retrieval, stats | `tenant.slug` matches, `endpoints` present |
| 2.3 | Update tenant | `PATCH /api/v1/agent/:slug` | Partial update, allowed domains | `success == true`, updated fields reflected |

**Note:** Delete test removed from Group 2 — it breaks the test chain for Groups 3-9. Cleanup is handled by Group 10.
...
**Import endpoint — exact curl invocation (form data, NOT JSON):**
```bash
# Import KB.MD
...
- `audience_type` — `"external"` or `"internal"`
- `file_type` — `"kb"` or `"policy"`
- `file` — the .md file (multipart upload)
...
| 4.2 | Set LLM config | `PUT /api/v1/agent/:slug/llm-config` | Config update | Response 200 |

**Note:** This group runs BEFORE chat/simulation tests (Groups 6-8). If LLM config is missing, those tests will silently fail.
...
| 5.2 | RAG search explorer | `POST /api/v1/agent/:slug/rag/search` | Vector search, scoring | `results` is array |
...
| 6.2 | Follow-up message | `POST /api/v1/agent/:slug/chat/ext` | Session continuity, context | `session_id` matches previous, `message.content` present |
...
| # | Test | API Endpoint | Validates | JSON Assertions |
|---|------|-------------|-----------|-----------------|
| 9.1 | List permissions | `GET /api/v1/agent/:slug/permissions` | Permission retrieval | Response is array or object with permissions |
| 9.2 | Grant permission | `POST /api/v1/agent/:slug/permissions` | User permission assignment | `success == true` or 200 |
...
| # | Test | API Endpoint | Validates | JSON Assertions |
|---|------|-------------|-----------|-----------------|
| 11.1 | Delete test tenant | `DELETE /api/v1/agent/:slug` | Cascade deletion | `success == true` or HTTP 200 |
...
- `SLUG` variable for test tenant slug
...
- `cleanup_tenant()` — delete tenant if exists (for Group 11)
...
The setup flow in `run_all.sh` uses `GET /healthz` to wait for the server:
...
7. **Re-run safety**: Auth tests handle existing users gracefully (fallback to sign-in)
