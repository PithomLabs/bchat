...
**Date:** 2026-07-14
**Status:** Ready for approval (incorporated review_ux.md findings)
...
Backend defines typed constants (review nit #2):
...
// Pre-check: source files exist (cheap DB query)
...
func (s *Service) createFailedCheckpoint(ctx context.Context, tenantID int32, audience, msg string) {
    ...
    // Detached context: checkpoint write must succeed even if parent is cancelled.
...
**Note (review nit #3):** Log the error instead of silently swallowing it. Leave the field
empty on failure — the frontend treats omission as "unknown" and shows nothing.
...
**Why:** Category-appropriate toast for every skip reason. No more false "started" toasts.
...
**Note (review minor):** Uses same container styling as the existing progress card for
visual consistency. Could be swapped to Joy UI `Alert` component in a follow-up.
...
**Note (review nit #4):** All 5 keys are now referenced by the frontend toast switch (Change 5)
and idle messaging (Change 6). No unused keys.
...
Add test cases:
- `TestHandleReindex_LongContextSkip` — mock tenant with long_context mode, verify 200 + skip_reason
- `TestHandleReindex_NoSourceFiles` — mock tenant with no files, verify 200 + skip_reason
- `TestHandleReindex_PipelineDisabled` — mock service with NoOpVectorDB, verify 200 + skip_reason

**File:** `server/router/api/v1/agent/service_reindex_test.go`

Add test cases:
- `TestCreateFailedCheckpoint` — verify checkpoint is written with status "failed" and correct message
- `TestReindexPipelineDisabled_CreatesCheckpoint` — verify failed checkpoint when NoOpVectorDB

---

## User Experience Matrix (updated per review)

| Scenario | HTTP Status | Toast | Progress Card | Detection |
|----------|-------------|-------|---------------|-----------|
| long_context skip | 200 OK | ℹ️ "Skipped — long_context mode" | Blue info: "Full document sent to LLM" | Synchronous |
| No source files | 200 OK | ⚠️ "No files to index" | — | Synchronous |
| Pipeline disabled | 200 OK | ⚠️ "Pipeline not configured" | — | Synchronous |
| Normal reindex start | 202 Accepted | ✅ "Reindexing started" | Progress bar + chunk count | Async |
| Zero chunks from chunking | 202 Accepted | ✅ "Reindexing started" | Checkpoint: 0 chunks | Async |
| Goroutine fails early | 202 Accepted | ✅ "Reindexing started" | Failed checkpoint → red | Async |
| Embedding provider down | 202 Accepted | ✅ "Reindexing started" | Failed checkpoint → red | Async |
| Idle, RAG mode, no index | — | — | Amber: "Upload files first" | Poll |
| Idle, long_context | — | — | Blue info: "No index needed" | Poll |

**Key improvement from review:** Pre-check failures (rows 2-3) now return synchronously
with appropriate toasts instead of misleading "started" + async failure.
...
|------|---------|----------------|
| `handlers.go` | Add skip_reason constants, 3 synchronous pre-checks, use constants in long_context skip | ~1172-1191 (new pre-checks), top of file (constants) |
| `service.go` | Add `createFailedCheckpoint` helper, call for G1-G2, add `retrieval_mode` to `ReindexStatus`, populate in `GetReindexStatus` | ~517-528, ~711-718, ~585-587 |
| `AgentAdmin.tsx` | Handle skip_reason in toast (switch), add idle-state messaging | ~302-316, ~1234-1294 |
| `agentAdmin.ts` | Add SkipReason type, add `retrieval_mode` to ReindexStatus, return skip_reason | ~252-261, ~963-985 |
| `en.json` | Add 5 new translation keys | After line 181 |
| `handlers_test.go` | Add 3 pre-check test cases | New tests |
| `service_reindex_test.go` | Add 2 checkpoint test cases | New tests |
...
| Extra DB query for source file count in handler | Low — single indexed query, fast | Amortized by eliminating unnecessary goroutine |

---

## Rollback

Revert the 7 file changes. No data model changes. No migrations needed.
