...
9. `INSERT OR IGNORE` → `INSERT ... ON CONFLICT DO NOTHING` (applies to LATEST.sql seed data only, e.g., `tenant_role_templates`)
...
### Step 3: Verify Fly.io app exists
```bash
fly apps list
# If your app is not listed, create it first:
# fly launch
```
