...
- **File:** `.env:16`, `.env:38`
- `OPENROUTER_API_KEY=sk-or-v1-ad851434...` and `ENCRYPTION_MASTER_KEY=e2590f42-...` are present in the repository working tree. While `.env` is correctly listed in `.gitignore:92` and is not tracked in git, these credentials are visible to anyone with filesystem access.
