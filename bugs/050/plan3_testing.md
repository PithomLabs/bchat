...
docker exec -it bchat-crdb cockroach sql --insecure --host=localhost -d bchat \
  -e "CREATE VECTOR INDEX IF NOT EXISTS idx_agent_vectors_embedding ON agent_vectors USING vector (embedding);"
```
...
Review `plan3_testing.md` for correctness, completeness, and risks. Check:
...
2. **Vector Index DDL:** Is `CREATE VECTOR INDEX IF NOT EXISTS idx_agent_vectors_embedding ON agent_vectors USING vector (embedding)` correct syntax for CRDB v25.2?
