...
| **Vector index syntax** | `CREATE VECTOR INDEX idx ON t (embedding vector_ip_ops)` | `CREATE INDEX idx ON t USING hnsw (embedding vector_cosine_ops)` |
...
```sql
-- Skills repo only documents vector_ip_ops (01-schema-design.md:152)
-- vector_cosine_ops is NOT documented in the skills repo
-- Cosine distance on normalized vectors = inner product, so vector_ip_ops produces identical results
CREATE VECTOR INDEX idx_agent_vectors_embedding ON agent_vectors (embedding vector_ip_ops);
```
...
**Search query** (from `cockroachdb-skills-main/.../03-query-patterns.md:331-334`):
```sql
...
func (v *CockroachVectorDB) Validate(ctx context.Context) error {
    ...
    // NOTE: CREATE VECTOR INDEX does NOT support IF NOT EXISTS (verified against 01-schema-design.md:152)
    // Check for "relation already exists" (pgcode 42P07) and treat as success
    _, err = v.db.ExecContext(ctx, `
        CREATE VECTOR INDEX idx_agent_vectors_embedding
        ON agent_vectors (embedding vector_ip_ops)
    `)
    if err != nil {
        // Check if the index already exists (pgcode 42P07 = duplicate_table)
        var pgErr *pq.Error
...
### Connection Wiring — Post-Construction Pattern

**Problem:** `NewVectorDB()` factory (`vectordb.go:256`) takes only `*VectorDBConfig` — no `*sql.DB`, no `store.Store`. The factory doesn't have access to the application's shared connection pool.

**Solution:** Use post-construction wiring like the existing `TenantVectorDBPool.SetStore()` pattern (`vectordb_pool.go:44`, `service.go:153,512`).

```go
// CockroachVectorDB implements SetDB for post-construction wiring
func (v *CockroachVectorDB) SetDB(db *sql.DB) {
    v.db = db
}

// In service.go NewService(), after NewVectorDB():
...
// Wire the existing *sql.DB if COCKROACH_DSN matches or is empty
...
func (v *CockroachVectorDB) Search(ctx context.Context, query SearchQuery) (*SearchResult, error) {
    ...
    // 3. Build query with filters (use $1::VECTOR cast for safety)
    // SELECT id, title, content, content_type, metadata, source_version, created_at,
    //        1 - (embedding <=> $1::VECTOR) AS similarity
    // FROM agent_vectors
    // WHERE tenant_id = $2 AND content_type = ANY($3)
...
The `agent_vectors` table is created by `CockroachVectorDB.Validate()` at runtime and is excluded from migration-based schema validation tasks (`task validate:schema`, `task validate:parity`). This is intentional — the table is CRDB-only and provider-managed.
...
**FIX (C-2):** The original plan referenced undefined `tenantID` in the cron closure. The `NewService()` constructor operates on the global store, not a single tenant. The cron job must iterate all active tenants.
...
func NewService(s *store.Store, p *profile.Profile) *Service {
    ...
    if os.Getenv("VECTOR_DB_PROVIDER") == "cockroach" {
        ...
        cronJob.AddFunc("*/5 * * * *", func() {
            svc.processPendingTickets(context.Background())
        })
        cronJob.Start()
    }
...
// processPendingTickets iterates all active tenants and processes pending tickets.
func (s *Service) processPendingTickets(ctx context.Context) {
    ...
    for _, tenant := range tenants {
        ...
        if err := s.ticketEmbedder.ProcessPending(ctx, tenant.ID); err != nil {
            slog.Error("failed to process pending tickets",
                "tenant_id", tenant.ID,
                "slug", tenant.Slug,
                "error", err)
        }
...
# NOTE: --spend-limit 0 disables the monthly spend cap. For hackathon demos,
# consider setting a budget or running `task crdb:cluster:delete` after demo.
...
**Performance note (M-1):** Batch inserts in groups of 10 per transaction to reduce round trips from 50 to 5.
...
| `service.go` | Add vector search to response generation, add `processPendingTickets()` | ~30 |
...
1. **CockroachDB cluster** — Vector index may require v25.2+ or a preview feature flag. Verify against official CRDB docs before deployment.
...
```yaml
  ...
  crdb:cluster:create:
    ...
    # Source: ccloud.md:47-70 — "ccloud cluster create basic"
    # NOTE: --spend-limit 0 disables the monthly spend cap
    cmds:
  ...
  crdb:db-check:
    ...
    cmds:
      ...
      - |
        ...
        echo "NOTE: agent_vectors table is created by Validate() at runtime"
        echo "      and is excluded from migration-based schema validation"
...
```yaml
  ...
  crdb:sql:query:
    ...
    # NOTE: Uses ccloud sql shell, not psql (psql has CRDB compatibility issues with simple_protocol)
    cmds:
      - |
        if [ -f .env ]; then
          set -a && . .env && set +a
        fi
        echo "{{.SQL}}" | ccloud cluster sql {{.NAME | default "bchat-db"}}
```
...
Switching `VECTOR_DB_PROVIDER` between `local`, `s3`, and `cockroach` creates a fresh namespace. Data in the previous provider is preserved but not accessible through the new provider. For the hackathon demo, ensure the correct provider is set before seeding data.

---

## Changes from plan3.1.md (Review Fixes)

| Finding | Severity | Fix | Evidence |
|---------|----------|-----|----------|
| **C-1**: `CREATE VECTOR INDEX IF NOT EXISTS` unsupported | CRITICAL | Remove `IF NOT EXISTS`, check `pgcode == "42P07"` | `01-schema-design.md:152` — no `IF NOT EXISTS` variant |
| **C-2**: Cron captures undefined `tenantID` | CRITICAL | Iterate all active tenants via `store.ListAgentTenants()` | `service.go:88-96` — `NewService()` has no tenant scope |
| **H-1**: Factory lacks `*sql.DB` | HIGH | Add `SetDB()` method, post-construction wiring | `vectordb.go:256`, `vectordb_pool.go:44`, `service.go:153` |
| **H-2**: `vector_cosine_ops` unverified | HIGH | Use `vector_ip_ops` (only verified opclass) | `01-schema-design.md:152` — only `vector_ip_ops` |
| **H-3**: `feature.vector_index.enabled` not found | HIGH | Remove from prerequisites, add version note | Grep: 0 matches in skills repo + bchat |
| **H-4**: Embedding JSON serialization | HIGH | Use `$1::VECTOR` cast in SQL | `03-query-patterns.md:333` — `$1::VECTOR` cast |
| **M-1**: Seed 50 single-row transactions | MEDIUM | Batch in groups of 10 | Latency reduction |
| **M-2**: `psql` fragility | MEDIUM | Use `ccloud cluster sql` instead | `simple_protocol` is for pgx, not psql |
| **M-3**: LanceDB orphan data | MEDIUM | Add documentation note | No code change |
| **M-5**: Spend limit cost | MEDIUM | Add warning: `--spend-limit 0` = no cap | `ccloud.md:69` |
| **M-6**: Migration invisibility | MEDIUM | Add note to `crdb:db-check` task | No code change |
