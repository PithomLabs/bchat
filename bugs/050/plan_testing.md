# Plan: Local CockroachDB Testing with Docker
...
| ~1 GB disk for image | `cockroachdb/cockroach` is ~800 MB | |
...
# Or pin: docker pull cockroachdb/cockroach:v25.2.3
```
...
services:
  cockroach:
    ...
    healthcheck:
      test: ["CMD", "cockroach", "node", "status", "--insecure", "--host=localhost"]
      interval: 5s
      timeout: 5s
      retries: 5
...
```bash
# Unit tests (skip real CRDB tests — they use t.Skip)
...
| `relation "agent_vectors" does not exist` | Schema not auto-created | Run bchat once (it creates schema on startup) |
| `vector index not supported` | CRDB version too old | Use `v25.2.x` or later |
...
| Vector index | Same syntax | Same syntax |
...
2. Run `task run:cockroach:seed` with demo data
3. Test full chat flow against local CRDB
4. Provision real CRDB cluster (`deploy/ccloud/setup.sh`)
5. Switch DSN to real cluster
6. Verify vector search works end-to-end

---

## Adversarial Plan Review Prompt

Review `plan_testing.md` for correctness, completeness, and risks. Check:

1. **Docker Image:** Is `cockroachdb/cockroach:v25.2.3` a valid image? Check Docker Hub for latest stable CRDB version.
2. **Vector Index Support:** Does CRDB v25.2 support `CREATE VECTOR INDEX` natively, or does it require a preview flag? Check CRDB docs.
3. **DSN Format:** Is `postgresql://user:pass@host:26257/db?sslmode=disable` correct for pgx v5 + CRDB insecure mode?
4. **Schema Auto-Creation:** Does bchat auto-create `agent_vectors` table on startup, or is manual schema setup required? Check `vectordb_cockroach.go` for `CREATE TABLE IF NOT EXISTS`.
5. **Port Conflicts:** Are 26257 and 8080 the correct default ports for CRDB single-node?
6. **Docker Compose Healthcheck:** Is `cockroach node status --insecure --host=localhost` valid inside the container?
7. **Missing Steps:** Any prerequisites or steps missing (e.g., Docker Compose version, network config)?
8. **Security:** Is `--insecure` safe for local dev? Any gotchas with Docker networking?
9. **Data Persistence:** Does `-v bchat_crdb_data:/cockroach/cockroach-data` persist data correctly across container restarts?
10. **Alternative Approach:** Should we use the CRDB binary directly (as in `setting-up-local-cluster/SKILL.md`) instead of Docker? Pros/cons?
