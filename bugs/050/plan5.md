...
func (v *CockroachVectorDB) Validate(ctx context.Context) error {
    ...
    if err != nil {
        ...
        // Use pgconn.PgError (pgx/v5) — NOT pq.Error (lib/pq is banned per AGENTS.md)
...
**FIX (C-2 from plan3.1 review):** The original plan referenced undefined `tenantID` in the cron closure. The `NewService()` constructor operates on the global store, not a single tenant. The cron job must iterate all active tenants.
...
// Filters at DB level using IsActive field (store/agent.go:28-33).
...
**Performance note (M-1 from plan3.1 review):** Batch inserts in groups of 10 per transaction to reduce round trips from 50 to 5.
...
## Changes from plan4.md (Review Fixes)

| Finding | Severity | Fix | Evidence |
|---------|----------|-----|----------|
| **C-4**: `pq.Error` uses banned `lib/pq` | CRITICAL | Replace with `pgconn.PgError` (import `github.com/jackc/pgx/v5/pgconn`) | AGENTS.md line 32, `resilience.go:18,42` |
| **N-1**: `$1::VECTOR` case inconsistency | INVALID | Plan4 already uses `::VECTOR` (uppercase) matching skills repo | `03-query-patterns.md:333,343` uses uppercase |
| **N-2**: `processPendingTickets()` can filter at DB level | NIT | Pass `IsActive: &[]bool{true}[0]` to `FindAgentTenant` | `agent.go:28-33` has `IsActive *bool` field |
| **N-3**: DSN comparison may fail for SQLite | NIT | Add comment clarifying `SetDB` is only for Postgres-based deployments | `profile.go:92-95` — SQLite DSN is file path |
