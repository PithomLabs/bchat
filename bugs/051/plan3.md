# Plan3: Internal Notes + RAG-Based Bug Inference (Final)

**Bug ID:** 051
**Date:** 2026-07-30
**Status:** Final — Post-Plan2 Review
**Revision:** plan3.md (incorporates N1-N5 from plan2_review.md)

---

## Changelog: plan2.md → plan3.md

| Finding | Source | Resolution |
|---------|--------|------------|
| N1. Agent Service Call Chain | plan2_review.md N1 | **Fixed** — Use `s.agentHandler.GetService().inferResolutionForNewTicket(ctx, ticket)` |
| N2. `HasPermission` Helper Omitted from Step 6 | plan2_review.md N2 | **Fixed** — Added as sub-task in step 6 |
| N3. Import/CreateTicket `internal_notes` Conflict | plan2_review.md N3 | **Fixed** — Import creates ticket (internal_notes=""), then immediately calls UpdateTicket to set "Pending summary..." |
| N4. Raw SQL vs Existing Search API | plan2_review.md N4 | **Fixed** — Use `vectorDB.Search(ctx, SearchQuery{...})` matching existing pattern at service.go:5568 |
| N5. Import Interrupt Resilience | plan2_review.md N5 | **Fixed** — Documented as known limitation with manual cleanup command |
...
hasInternalNotesPerm := agent.HasPermission(resolvedPerms, agent.PermTicketInternalNotes)
```

#### `HasPermission` Helper

Add to `permissions.go` (public wrapper around unexported `containsResolvedPermission`):

```go
// HasPermission checks if a resolved permission set contains a required permission.
...
**Note:** `ContainsPermission` (line 65) is deprecated but works with `[]string`. `HasPermission` uses the newer `[]ResolvedPermission` type returned by `ResolveEffectivePermissions`.
...
- Import script uses a two-step process (see Section 4)
...
Phase 1: Create Tickets (synchronous)
  For each bug folder (001-050):
    ...
    3. For each phase (plan, code, testing):
       a. Create memo with phase content
       b. Create ticket linked to memo (internal_notes = "")
       c. Immediately update ticket: internal_notes = "Pending summary..."
...
Phase 2: Generate LLM Summaries (async background)
  ...
  Interrupt Resilience (N5):
  - Re-running the import detects tickets with internal_notes = "Pending summary..."
  - Skips those and re-processes only pending tickets
...
**Call chain (N1 fix):** `APIV1Service` → `agentHandler *agent.Handler` → `GetService() *Service` → `inferResolutionForNewTicket()`. Pattern confirmed at `memo_service.go:1225`.
...
func (s *Service) inferResolutionForNewTicket(ctx context.Context, ticket *store.Ticket) {
    ...
    //    Uses existing Search() API (N4 fix) — not raw SQL
    ...
    if err != nil {
        slog.Error("failed to search for similar tickets", "error", err)
        return
    }
...
**Pattern reference:** Existing vector search call at `service.go:5568`:
```go
...
| 6 | Permission constant + presets + `HasPermission` helper | `server/router/api/v1/agent/permissions.go` | 15 min |
...
| 9a | Bug import script (create tickets) | `cmd/import-bugs/main.go` | 2-3 hours |
| 9b | Bug import script (async LLM summaries) | `cmd/import-bugs/main.go` | (included in 9a) |
...
| `TestImportResume` | `import_bugs_test.go` | Re-run skips already-summarized tickets |
...
### Import Interrupt Resilience (N5)

If the import process is killed mid-Phase-2:
- Tickets exist with `internal_notes = "Pending summary..."`
- Re-running the import detects and re-processes only pending tickets
- Manual cleanup if needed:
  ```sql
  UPDATE tickets SET internal_notes = '' WHERE internal_notes = 'Pending summary...';
  ```

### Internal Notes in Vector Search (Finding #9)

Internal notes content is included in ticket embeddings for richer semantic search. This means:
- Vector search can surface internal notes patterns across tickets
- RBAC is enforced at API response level, not at vector search level
- Acceptable for hackathon demo; production systems may want a separate embedding field
...
- ContainsPermission (deprecated): `/home/chaschel/Documents/go/bchat/server/router/api/v1/agent/permissions.go:65`
- containsResolvedPermission (unexported): `/home/chaschel/Documents/go/bchat/server/router/api/v1/agent/permissions.go:84`
...
- Existing vector search pattern: `/home/chaschel/Documents/go/bchat/server/router/api/v1/agent/service.go:5568`
- Existing agent call chain: `/home/chaschel/Documents/go/bchat/server/router/api/v1/memo_service.go:1225`
