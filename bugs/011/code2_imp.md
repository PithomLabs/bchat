Implementation complete. Verified the existing codebase already had the core fixes (superuser bypass in `ListMemoRelations`, `forceVisibility`/`hideVisibilitySelector` props in `MemoEditor`, and proper prop usage in `Tickets.tsx`). Applied one nit fix to eliminate a render flash by initializing `memoVisibility` from `forceVisibility ?? defaultVisibility` in `MemoEditor/index.tsx:86`.
