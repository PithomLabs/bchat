# Code Documentation: Per-Ticket RAG Indexing Implementation (Revised)

**Bug ID:** 052
**Date:** 2026-07-31
**Status:** Implemented — Compiles, Tests Pass
**Revision:** Based on adversarial code review of code.md
...
| `server/router/api/v1/memo_service.go` | +104 | 0 | +104 |
| `server/router/api/v1/memo_service_test.go` | +310 | 0 | +310 |
| **Total** | **+586** | **-7** | **+579** |
...
**Known limitation:** Entries are never removed. For a prototype with a bounded
number of tickets, this is acceptable. For production, a background cleanup
goroutine with TTL-based eviction should be added, or the dedup logic should be
moved inside `ReindexFileVersion` to eliminate the per-ticket map entirely.
...
**Known limitation (N+1):** For each comment, a separate `GetMemo` query is issued.
For 100 comments, this produces 100 round-trips. Post-prototype optimization:
batch-fetch all comment memos using `ListMemos` with `ID IN (...)`.
    ...
    if tenantID := GetTenantIDFromContext(ctx); tenantID != nil {
        ...
        if relErr == nil && len(parentRelations) > 0 {
            ...
            for _, rel := range parentRelations {
                ...
                // ... (see full code above)
                // ListTickets error logged before continue
...
| `ListTickets` fails in hooks | Log warning, return/continue |
| `ListMemoRelations` fails in UpdateMemo | Log debug, skip reindex block |
...
## 9. Known Issues (Post-Prototype Fixes)

| # | Issue | Severity | Location | Fix |
|---|-------|----------|----------|-----|
| 1 | `ticketIndexMu` grows unboundedly | MEDIUM | `service.go:5702` | Add TTL-based cleanup goroutine or move dedup into `ReindexFileVersion` |
| 2 | N+1 queries in `getTicketComments` | NIT | `ticket_service.go:546-584` | Batch-fetch with `ListMemos` + `ID IN (...)` |
...
## 12. Adversarial Code Review Prompt

```
You are an adversarial code reviewer performing a final code review of the
Per-Ticket RAG Indexing implementation (bugs/052, code2.md revision). The code
has been implemented, compiles cleanly, and addresses logging gaps from the
prior review. Review the following three files:
...
1. LOGGING: Are all error paths now properly logged? Verify that no `_, _ :=`
   or `tickets, _ :=` patterns remain without error logging.

2. CORRECTNESS: Does the content-hash dedup logic handle all edge cases?
   Is the per-ticket mutex correctly scoped?

3. SECURITY: Are all tenant isolation guards correct? Can a nil TenantID
   slip through any path?

4. CONCURRENCY: Is the goroutine capture-by-value pattern correct in the
   UpdateMemo hook?

5. KNOWN ISSUES: Are the 5 known issues from Section 9 properly documented
   and tracked for post-prototype resolution?
