# Plan 4: Bug 056 — "parent memo not found" Error (Final)

**Date:** 2026-08-01  
**Status:** Ready for implementation  
**Revision:** Incorporates all findings from `plan3_bug_review.md`  
...
| 7 | `convertMemoRelationFromStore` can nil-dereference when relation's tenant-scoped endpoint is a legacy memo | `memo_relation_service.go` converter | MEDIUM |
...
Add two helpers. The first is for ticket-internal paths that have a concrete `tenantID int32` but no user object. The second is for gRPC memo APIs that have both user and tenant context, preserving existing superuser behavior.
...
//
// Returns false for nil memo (caller must handle separately).
func MemoIsAccessible(memo *store.Memo, user *store.User, tenantID *int32) bool {
    if memo == nil {
        return false
    }
    if memo.TenantID == nil {
...
**Rationale for two helpers:** Ticket-internal goroutines (`createSystemResolutionComment`, `getTicketComments`) do not carry a `*store.User` and must not silently grant superuser access. gRPC memo APIs (`CreateMemoComment`, `ListMemoComments`, `SetMemoRelations`, `ListMemoRelations`) run in authenticated request contexts where superuser behavior is already expected and must be preserved.
...
func (s *APIV1Service) CreateMemoComment(ctx context.Context, request *v1pb.CreateMemoCommentRequest) (*v1pb.Memo, error) {
    ...
    if !MemoIsAccessible(relatedMemo, user, GetTenantIDFromContext(ctx)) {
        return nil, status.Errorf(codes.NotFound, "memo not found")
    }
...
func (s *APIV1Service) SetMemoRelations(ctx context.Context, request *v1pb.SetMemoRelationsRequest) (*emptypb.Empty, error) {
    ...
    user := getUserFromContext(ctx)
    if !MemoIsAccessible(memo, user, GetTenantIDFromContext(ctx)) {
        return nil, status.Errorf(codes.NotFound, "memo not found")
    }
...
func (s *APIV1Service) ListMemoRelations(ctx context.Context, request *v1pb.ListMemoRelationsRequest) (*v1pb.ListMemoRelationsResponse, error) {
    ...
    // ... rest unchanged (ListMemoRelations by memo.ID, then convert each relation) ...
}
```
...
Update the converter to handle nil memo results after ID lookups. When a relation's `TenantID` points to a legacy memo (`tenant_id=NULL`), the ID+tenant lookup returns nil. The converter must not dereference nil.
...
func (s *APIV1Service) convertMemoRelationFromStore(ctx context.Context, memoRelation *store.MemoRelation) (*v1pb.MemoRelation, error) {
    ...
    return &v1pb.MemoRelation{
        ...
        RelatedMemo: &v1pb.MemoRelation_Memo{
            Name:    fmt.Sprintf("memos/%s", relatedMemo.UID),
            Uid:     relatedMemo.UID,
            Snippet: relatedMemo.Content,
        },
...
**Rationale:** The `ListMemoRelations` handler already verified the source memo is accessible before calling the converter. Removing the tenant filter from the converter's internal lookups ensures legacy memos are not silently dropped during response construction. Ownership was already enforced at the handler level.
...
| File | Changes |
|------|---------|
| `server/router/api/v1/tenant_context.go` | Add `MemoBelongsToTenantOrLegacy` and `MemoIsAccessible` helpers |
...
| `server/router/api/v1/memo_relation_service.go` | Fix `SetMemoRelations` (UID-only lookups + user-aware ownership checks for source and related memos + correct proto field access), fix `ListMemoRelations` (UID-only lookup + user-aware ownership check), fix `convertMemoRelationFromStore` (remove tenant filter from ID lookups + nil safety) |
...
#### Test 15: `convertMemoRelationFromStore` with legacy endpoint memo
...
#### Test 16: Superuser can access tenant-scoped memo via `SetMemoRelations`
...
#### Test 17: Superuser can access tenant-scoped memo via `ListMemoRelations`
...
2. **Two-tier access control**: `MemoBelongsToTenantOrLegacy` enforces strict tenant matching for ticket-internal paths where no user context exists. `MemoIsAccessible` preserves existing superuser bypass behavior for gRPC memo APIs, matching the system's current `GetMemo` semantics.
...
5. **Fixes converter nil-dereference**: `convertMemoRelationFromStore` no longer applies tenant filters to its internal ID lookups, preventing nil dereference when a tenant-scoped relation points to a legacy memo endpoint.

6. **Defensive guards**: Empty `/m/` descriptions and `/m/`-prefixed empty UIDs are handled cleanly before any store lookup.

7. **No data migration needed**: Purely in the query/logic layer — no schema changes, no reindex, no data backfill.

8. **Regression tests**: Seventeen targeted tests cover legacy memos, cross-tenant rejection, tenant-scoped relations, superuser access, nil-tenant relation endpoints, and edge cases across all touched paths.
