# Plan 5: Bug 056 — "parent memo not found" Error (Final)

**Date:** 2026-08-01  
**Status:** Ready for implementation  
**Revision:** Incorporates all findings from `plan4_bug_review.md`  
...
| 7 | Empty `/m/` descriptions produce needless lookups | `createSystemResolutionComment`, `getTicketComments` | MEDIUM |
| 8 | `fmt.Errorf` wraps nil error producing `%!w(<nil>)` | `createSystemResolutionComment` (line 643) | LOW |
| 9 | Error message leaks memo UID | `createSystemResolutionComment` (line 643) | LOW |
...
Add three helpers. The first is for ticket-internal paths that have a concrete `tenantID int32` but no user object. The second is for gRPC memo APIs that have both user and tenant context, preserving existing superuser and scoped-admin behavior. The third derives the effective tenant ID for relation operations.
...
// the request has no tenant context (e.g., unscoped superuser/admin).
...
- `MemoBelongsToTenantOrLegacy`: strict tenant-only check for ticket-internal goroutines where no user object exists and superuser access must not be silently granted.
- `MemoIsAccessible`: user-aware check for gRPC memo APIs, preserving `GetMemo` semantics including scoped-admin restrictions on legacy memos.
- `RelationTenantID`: ensures relation operations use the request tenant when present, preventing cross-tenant relation deletion/exposure for legacy source memos.
...
#### Test 13: `SetMemoRelations` with legacy source memo deletes only request tenant's relations
...
#### Test 14: `ListMemoRelations` with legacy source memo returns only request tenant's relations
...
#### Test 15: `ListMemoRelations` with legacy source memo
...
#### Test 16: `ListMemoRelations` cross-tenant rejection
...
#### Test 17: `convertMemoRelationFromStore` with legacy endpoint memo
...
#### Test 18: Superuser can access tenant-scoped memo via `SetMemoRelations`
...
#### Test 19: Superuser can access tenant-scoped memo via `ListMemoRelations`
...
#### Test 20: Scoped admin cannot access legacy memo via `SetMemoRelations`
...
9. **Regression tests**: Twenty targeted tests cover legacy memos, cross-tenant rejection, tenant-scoped relations, superuser access, scoped-admin restrictions, relation isolation, nil-tenant relation endpoints, and edge cases across all touched paths.
