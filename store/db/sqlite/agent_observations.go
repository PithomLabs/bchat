...
func (d *DB) UpsertObservationLog(ctx context.Context, log *store.ObservationLog) (*store.ObservationLog, error) {
	stmt := `
		INSERT INTO agent_observations (
			session_id, tenant_id, observation_log, last_observed_msg_index, tokens_in_log, last_updated_at
		)
		VALUES (?, ?, ?, ?, ?, ?)
	...
	if err := d.db.QueryRowContext(ctx, stmt,
		log.SessionID, log.TenantID, log.ObservationLog, log.LastObservedMsgIndex, log.TokensInLog, log.LastUpdatedAt,
...
func (d *DB) GetObservationLog(ctx context.Context, sessionID string) (*store.ObservationLog, error) {
	stmt := `
		SELECT session_id, tenant_id, observation_log, last_observed_msg_index, tokens_in_log, created_at, last_updated_at
	...
	if err := row.Scan(
		&log.SessionID, &log.TenantID, &log.ObservationLog, &log.LastObservedMsgIndex, &log.TokensInLog, &log.CreatedAt, &log.LastUpdatedAt,
	); err != nil {
