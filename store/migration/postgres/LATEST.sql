...
CREATE UNIQUE INDEX idx_reindex_checkpoint_tenant_audience ON agent_reindex_checkpoints(tenant_id, audience, file_type, version);
...
CREATE UNIQUE INDEX idx_rag_active_version_lookup ON agent_rag_active_versions(tenant_id, audience_type, file_type);
...
CREATE UNIQUE INDEX idx_bridge_handoffs_one_active ON bridge_handoffs(external_session_id) WHERE active = TRUE;
